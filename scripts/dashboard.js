        ...
        value: () => {
            ...
            databaseMethods.getSongs().then((responseSongs) =>
                responseSongs.forEach(element => {
                    ...
                    // const divRef = document.querySelector(".div--container");
                    const $pSongTitle = $("<p>").text(`${element.title}`).appendTo($artRef)
                    const $pSongAlbum = $("<p>").text(`${element.album}`).appendTo($pSongTitle)
                    const $pSongArtist = $("<p>").text(`${element.artist}`).appendTo($pSongAlbum)
